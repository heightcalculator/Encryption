...
The `encrypt(message)` function takes the Plain Text string as an input and outputs the String of ciphertext numbers in Base 64
```js
const n = publicKey(p, q, e)[0]; //Remember to Hard Code this value so people don't have access to your primes, p and q
const e = publicKey(p, q, e)[1]; //Again, remember to Hard Code this value in
...
The `decrypt(message)` function takes the Cipher Text string which is a Base 64 number as the input and outputs the plaintext string in Base 64
